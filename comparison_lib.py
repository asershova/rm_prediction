...
def gold_dict_edit(gold_dict, f):
    if f == "Type_IV#R.gold":
        for i, j in [
                ["49#Query_1043",Domain(
                    hit_type='Non-specific', 
                    PSSM_ID='236618', 
                    start=1, stop=348, e_value=3.87155e-142, bitscore=409.539, accession='COG4268', short_name='McrC', incomplete='N', superfamily_PSSM_ID='382678', true_superfamily='cl01737')
                ],
                ["49#Query_1043",Domain(
                    hit_type='Non-specific', PSSM_ID='370821', start=13, stop=304, e_value=4.8546e-20, bitscore=88.8876, accession='pfam10117', short_name='McrBC', incomplete= '-', superfamily_PSSM_ID='382678', true_superfamily='cl01737')
                ]]:
            if i not in gold_dict:
                gold_dict[i] = Query(i.split("#")[0], ["xxx", i.split("#")[1],"zzz", "yyy", j.short_name])
                gold_dict[i].domains.append(j)
                gold_dict[i].best_domains.append([j])
            else:
                gold_dict[i].domains.append(j)
                gold_dict[i].best_domains.append([j])
            
    if f == "TypeII#R.gold":
        for i,j in ["10#Query_214",Domain(hit_type='Non-specific', PSSM_ID='372641', start=28, stop=169, e_value=1.25851e-17, bitscore=79.6608, accession='pfam13589', short_name='HATPase_c_3', incomplete= '-', superfamily_PSSM_ID='381791', true_superfamily='cl00075')], ["57#Query_1216", Domain(hit_type='Non-specific', PSSM_ID='372641', start=33, stop=157, e_value=1.36997e-14, bitscore=70.8013, accession='pfam13589', short_name='HATPase_c_3', incomplete= '-', superfamily_PSSM_ID='381791', true_superfamily='cl00075')]:
            if i not in gold_dict:
                gold_dict[i] = Query(i.split("#")[0], ["xxx", i.split("#")[1],"zzz", "yyy", j.short_name])
                gold_dict[i].domains.append(j)
                gold_dict[i].best_domains.append([j])
...
def print_best_scores (d_best_score, rm_type, out):
    w = csv.writer(open(out, "a"), delimiter = "\t")
    for key, val in d_best_score.items():
        w.writerow([key[0].split()[0], val.domains, val.best_rm_name, val.max_score, rm_type])
